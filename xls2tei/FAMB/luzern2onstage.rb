  ...
  CSV.foreach(file, {:encoding => "UTF-8"}) do |row|
    ...
    if hash_row[:doc_no] != nil
      if current_page != nil
        ...
        # ensembles: contains the shelfmark
        name = current_page[:shelfmark].gsub(" ", "_").gsub(".", "-") + "-"
        pages[name + current_page[:page]] = current_page
      end
      ...
      current_page = {
        ...
        shelfmark: hash_row[:names_ens], # we use the same field
        subpages: []
      }
    else
      ...
      current_page[:shelfmark] << hash_row[:shelfmark] if hash_row[:shelfmark]
